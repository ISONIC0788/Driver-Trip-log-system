...
INSERT INTO `activity_logs` (`activity_id`, `item_id`, `activity_name`, `time_occured`) VALUES
(0, 9, '', '2025-03-13 11:28:28'),
(1, 8, 'creted user ', '2025-03-13 09:36:12'),
(2, 9, 'creted user ', '2025-03-13 10:17:23'),
(3, 9, 'creted user ', '2025-03-17 17:11:21');
...
(6, 'Hellow', 'kigali', 'gashora ', 'kigali '),
(7, 'Kigali Heights  ', 'South province ', 'Bugesera', ' KG 7 Ave, Kigali'),
(8, 'Kigali Heights  ', 'South province ', 'Bugesera', ' KG 7 Ave, Kigali'),
(9, 'Kigali Heights  ', 'South province ', 'Bugesera', ' KG 7 Ave, Kigali');
...
(2, 2, 'hirwa edison ', '2025-03-12 14:56:35', 'approved'),
(3, 3, 'Sebarera patrcick', '2025-03-12 14:56:35', 'declined'),
(4, 4, 'ITUZE EBEDI Meleleck', '2025-03-12 14:56:35', 'pending'),
(5, 5, 'Charlie Davis', '2025-03-12 14:56:35', 'approved'),
(6, 1, 'ebebe', '2025-03-28 07:00:00', 'approved'),
(7, 7, 'Miriam Iriho', '2025-03-20 22:00:00', 'pending'),
(8, 4, 'kalisa', '2025-03-21 22:00:00', 'approved'),
(9, 5, 'ueheh', '2025-03-11 22:00:00', 'approved');
...
`trip_id` int(11)
,`vehicle_plate` varchar(150)
,`vehicle_type` enum('truck','bus','minibus','taxt')
,`driver_first_name` varchar(150)
,`driver_last_name` varchar(150)
...
(1, 'John', 'Doe', '40bd001563085fc35165329ea1ff5c5ecbdbbeef', 'john.doe@example.com', '0789123456'),
(2, 'Jane', 'Smith', '40bd001563085fc35165329ea1ff5c5ecbdbbeef', 'jane.smith@example.com', '0789234567'),
(3, 'Alice', 'Johnson', '40bd001563085fc35165329ea1ff5c5ecbdbbeef', 'alice.johnson@example.com', '0789345678'),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `trip_details_view`  AS  select `t`.`trip_id` AS `trip_id`,`v`.`plate_numbe` AS `vehicle_plate`,`v`.`vehicle_type` AS `vehicle_type`,`u`.`first_name` AS `driver_first_name`,`u`.`last_name` AS `driver_last_name`,`t`.`departure_time` AS `departure_time`,`t`.`departure_location` AS `departure_location` from (((`trips` `t` join `vehicles` `v` on((`t`.`vehicle_id` = `v`.`vehicle_id`))) join `drivers` `d` on((`t`.`driver_id` = `d`.`driver_id`))) join `users` `u` on((`d`.`user_id` = `u`.`user_id`))) ;
